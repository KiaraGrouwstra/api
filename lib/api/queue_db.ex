...
defdatabase QueueDb do
  ...
  deftable Queues, [:domain, :queue], type: :set do
    # @type t :: %Queue{domain: String.t, queue: Any}
  end

  # # this is just a forward declaration of the table, otherwise you'd have to fully scope User.read in Message functions
  # deftable User
  #
  # # this defines a table with an user_id key and a content attribute, and makes the table a bag; tables are basically records with a bunch of helpers
  ...
  # end
  #
  # # this defines a table with other attributes as ordered set, and defines an additional index as email, this improves lookup operations
  ...
  #
  #   # this is a helper function to add a message to the user, using write on the created records makes it write to the mnesia table
  #   def add_message(self, content) do
  #     %Message{user_id: self.id, content: content} |> Message.write
  #   end
  #
  #   # like above, but again with dirty operations, the bang methods are used throughout amnesia to be the dirty counterparts of the bang-less functions
  #   def add_message!(self, content) do
  #     %Message{user_id: self.id, content: content} |> Message.write!
  #   end
  #
